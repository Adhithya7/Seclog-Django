...
stdin,stdout,stderr=ssh.exec_command('sudo cat ~/aravind_files/testkeystone.log', get_pty= True)
time.sleep(2)
stdin.write(spass + '\n')
stdin.flush()
time.sleep(5)
...
with open(template_path + 'key_svfile.txt','r+') as fin1:
    if os.stat(template_path + 'key_svfile.txt').st_size == 0:
        m1 = hashlib.md5()
        fh_enc = str(fh[1:])
        i=0
        while True:
        ...
        output = open(template_path + 'key_check.txt','w+')  
