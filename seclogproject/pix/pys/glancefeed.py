...
opfile_path = template_path + 'output_glance.txt'
...
#stdin,stdout,stderr=ssh.exec_command('sudo cat /var/log/glance/api.log', get_pty= True)		#path to file glance log	
stdin,stdout,stderr=ssh.exec_command('sudo cat ~/testglance.log', get_pty= True)
time.sleep(2)
stdin.write(spass + '\n')
stdin.flush()
time.sleep(5)
...
#stdin,stdout,stderr=ssh.exec_command('sudo cat /var/log/glance/registry.log', get_pty= True)		#path to file glance log	
#stdin,stdout,stderr=ssh.exec_command('sudo cat ~/testglance.log', get_pty= True)
#time.sleep(2)
#stdin.write(spass + '\n')
#stdin.flush()
...
with open(template_path + 'gl_svfile.txt','r+') as fin1:
    if os.stat(template_path + 'gl_svfile.txt').st_size == 0:
        m1 = hashlib.md5()
        fh_enc = str(fh[1:])
        i=0
        while True:
        ...
        output = open(template_path + 'gl_check.txt','w+')  
