...
opfile_path = template_path + 'conf_output_neutron.txt'
...
stdin,stdout,stderr=ssh.exec_command('sudo cat ~/testneu.conf', get_pty= True)
#stdin,stdout,stderr=ssh.exec_command('sudo cat /etc/neutron/neutron.conf', get_pty= True)		#path to neutron.conf
time.sleep(2)
stdin.write(spass + '\n')
stdin.flush()
time.sleep(5)
...
#ssh.connect(hostname='%s' %host, username='%s' %uname, pkey=key, timeout=10)
#stdin,stdout,stderr=ssh.exec_command('sudo cat /etc/neutron/rootwrap.conf', get_pty= True)		#path to rootwrap.conf
#time.sleep(2)
#stdin.write(spass + '\n')
#stdin.flush()
#time.sleep(5)
