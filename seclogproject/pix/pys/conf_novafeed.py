...
opfile_path = template_path + 'conf_output_nova.txt'
...
stdin,stdout,stderr=ssh.exec_command('sudo cat ~/testnov.conf', get_pty= True)
#stdin,stdout,stderr=ssh.exec_command('sudo cat /etc/nova/nova.conf', get_pty= True)			#path to file nova.conf
time.sleep(2)
stdin.write(spass + '\n')
stdin.flush()
time.sleep(5)

rb = stdout.channel.recv(92)										#capture 100 bytes from socket
...
#ssh.connect(hostname='%s' %host, username='%s' %uname, pkey=key, timeout=10)
#stdin,stdout,stderr=ssh.exec_command('sudo cat /etc/nova/rootwrap.conf', get_pty= True)		#path to file rootwrap.conf
#time.sleep(2)
#stdin.write(spass + '\n')
#stdin.flush()
#time.sleep(5)
